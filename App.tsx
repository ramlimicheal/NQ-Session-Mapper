...
import { analyzeChartWithGemini, generateAIForecast } from './services/geminiService';
import {
...
const ReportTab: React.FC<{ dailyData: DailyData[], onExport: () => void }> = ({ dailyData, onExport }) => {
    ...
    return (
        <div className="bg-zinc-950/50 backdrop-blur-sm rounded-xl p-6 border border-zinc-800 animate-fadeIn">
             <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4">
                <h3 className="text-xl font-bold">📋 Trade Log Report (MNQ)</h3>
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-50 p-4 sm:p-6 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(120,119,198,0.2),rgba(255,255,255,0))]">
      ...
      <main className="max-w-7xl mx-auto">
        ...
        {activeTab === 'report' && (
            <ReportTab dailyData={results?.dailyData || []} onExport={exportReportCSV} />
        )}
      </main>
    </div>
  );
